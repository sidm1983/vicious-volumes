...
Today, we are not going to use any of the above methods. Instead, we are going to attempt to extract meaningful data from IIS log files using a developer-friendly approach. (Psst...this approach should work for any structured text file formats like CSV files, and not just IIS log files.)
